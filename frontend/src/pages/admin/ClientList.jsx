...
import { getAllBuyerBriefs, resetClientPassword, createClient, updateClientEmail, deactivatePortalUser, reactivatePortalUser } from '../../api/client';
...
const ClientList = () => {
  ...
  const filtered = clients.filter(c => {
    ...
    let matchesTab = true;
    if (activeTab === 'active')    matchesTab = c.portalUser?.status === 'onboarded';
    if (activeTab === 'inactive')  matchesTab = c.portalUser?.status === 'deactivated';
  ...
  const tabCounts = {
    ...
    inactive:  clients.filter(c => c.portalUser?.status === 'deactivated').length,
  ...
  const openCredentialsModal = (client) => {
    ...
    setCredentialForm({ email: client.portalUser?.email || client.email || '', password: '' });
  ...
  const handleUpdateCredentials = async (e) => {
    ...
    try {
      const originalEmail = editingClient.portalUser?.email || editingClient.email;
  ...
  const statusBadge = (client) => {
    if (!client.portalUser) {
      return <span className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border bg-gray-500/10 text-gray-400 border-gray-500/20">Unboarded</span>;
    }
    if (client.portalUser.status === 'onboarded') {
      return <span className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border bg-green-500/10 text-green-400 border-green-500/20">Active</span>;
    }
    return <span className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border bg-red-500/10 text-red-400 border-red-500/20">Inactive</span>;
  };
    ...
    <AdminLayout title="Client Management">
            ...
            {tabs.map(tab => (
              <button
                ...
                className={`px-5 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
                  activeTab === tab.value ? 'bg-teal text-navy' : 'text-gray-400 hover:text-white'
                }`}
              >
                {tab.label}
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${
                  activeTab === tab.value ? 'bg-navy/30 text-navy' : 'bg-white/10 text-gray-500'
                }`}>
                  {tabCounts[tab.value]}
                </span>
              </button>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {client.zohoContactId && (
                            <p className="text-[10px] text-gray-500 font-mono mt-0.5">{client.zohoContactId}</p>
                          )}
                        </div>
                      </div>
                    </td>
      ...
      {showCredentialsModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setShowCredentialsModal(false)} />
          <div className="relative bg-[#1B2A4A] border border-teal/30 rounded-3xl p-8 max-w-md w-full shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowCredentialsModal(false)} className="text-gray-500 hover:text-white">
                <X size={22} />
              </button>
            </div>
            <p className="text-gray-400 text-sm mb-6">
              Updating credentials for <span className="text-white font-bold">{editingClient?.fullName}</span>.
              Leave password blank to keep unchanged.
            </p>
            <form onSubmit={handleUpdateCredentials} className="space-y-5">
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest">Login Email</label>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button"
                    onClick={() => { setCredentialForm(f => ({ ...f, password: generatePassword() })); setShowCredPwd(true); }}
              ...
              <label className="flex items-center gap-3 cursor-pointer select-none group">
                <div className="relative">
                  ...
                  <div className="w-5 h-5 rounded-md border border-white/20 bg-navy peer-checked:bg-teal peer-checked:border-teal transition-all flex items-center justify-center">
                    {credSendEmail && <svg className="w-3 h-3 text-navy" fill="none" viewBox="0 0 12 12"><path d="M2 6l3 3 5-5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                  </div>
                </div>
                <div>
                  <p className="text-sm font-semibold text-white">Send Mail</p>
                  <p className="text-xs text-gray-500">Email updated credentials to the client</p>
                </div>
              </label>
      ...
      {showOnboardModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setShowOnboardModal(false)} />
          <div className="relative bg-[#1B2A4A] border border-teal/30 rounded-3xl p-8 max-w-md w-full shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-white">Onboard Client</h3>
              <button onClick={() => setShowOnboardModal(false)} className="text-gray-500 hover:text-white">
                <X size={22} />
              </button>
            </div>
            <p className="text-gray-400 text-sm mb-6">
              Creating portal access for <span className="text-white font-bold">{onboardingClient?.fullName}</span>.
            </p>
            <form onSubmit={handleOnboard} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    />
                    <button type="button" onClick={() => setShowOnboardPwd(v => !v)}
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-teal">
                      {showOnboardPwd ? <EyeOff size={15} /> : <Eye size={15} />}
                    </button>
                  </div>
                  <button type="button"
                    onClick={() => { setOnboardForm(f => ({ ...f, password: generatePassword() })); setShowOnboardPwd(true); }}
                    className="px-4 py-2 bg-teal/10 text-teal border border-teal/30 rounded-xl text-xs font-bold hover:bg-teal hover:text-navy transition-all whitespace-nowrap">
                    Generate
                  </button>
                </div>
              ...
              <button type="submit" disabled={onboarding}
                className="w-full py-3.5 bg-teal text-navy rounded-2xl text-sm font-bold hover:bg-teal/90 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                {onboarding ? <Loader2 className="animate-spin" size={18} /> : <><UserPlus size={16} /> Onboard Client</>}
              </button>
            </form>
          </div>
        </div>
