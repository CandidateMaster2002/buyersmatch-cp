...
import { getClientProperties, getStoredUser, getPropertyDocuments } from '../../api/client';
import Layout from '../../components/Layout';
import {
  Bed, Bath, Car, Loader2, Building2,
  Search, Scale, X, Check, ChevronDown, Lock,
  DollarSign, MapPin, Clock, Tag, Users,
  Percent, PiggyBank, Home, CheckCircle2, XCircle,
  FileText, Briefcase
} from 'lucide-react';
        ...
        <div className="flex-1 overflow-auto p-4 md:p-6">
          <div className="grid grid-cols-[120px_repeat(auto-fit,minmax(180px,1fr))] md:grid-cols-[150px_repeat(auto-fit,minmax(200px,1fr))] gap-3 md:gap-4 min-w-[600px] md:min-w-[800px]">
            ...
            <div className="space-y-3 md:space-y-4 pt-[140px] md:pt-[200px]">
              <div className="h-10 md:h-12 flex items-center text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest">Address</div>
              <div className="h-10 md:h-12 flex items-center text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest">Price Range</div>
              <div className="h-10 md:h-12 flex items-center text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest">Yield</div>
              <div className="h-10 md:h-12 flex items-center text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest">Bedrooms</div>
              <div className="h-10 md:h-12 flex items-center text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest">Bathrooms</div>
              <div className="h-10 md:h-12 flex items-center text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest">Car Parking</div>
              <div className="h-10 md:h-12 flex items-center text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest">Land Size</div>
              <div className="h-10 md:h-12 flex items-center text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest">Year Built</div>
              <div className="h-10 md:h-12 flex items-center text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest">Status</div>
            </div>
            ...
            {properties.map((item) => (
              <div key={item.id} className="space-y-3 md:space-y-4">
                <div className="h-[120px] md:h-[180px] rounded-xl md:rounded-2xl overflow-hidden border border-teal/20 mb-2 md:mb-4">
                  <img
                    src={item.firstImage || `https://placehold.co/400x300/1B2A4A/2ABFBF?text=IMG`}
    ...
    <Layout title="Dashboard">
      ...
      <div className="bg-teal/10 border border-teal/30 rounded-2xl p-6 lg:p-8 mb-8">
        <h1 className="text-3xl font-bold text-white mb-1">
          Welcome back, <span className="text-teal">{user?.greetingName || user?.fullName || 'Client'}</span>
        </h1>
        <p className="text-gray-400">Here's your Buyers Match portal overview.</p>
      </div>
        ...
        <div>
          {activeBriefs.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 bg-white/5 border border-dashed border-white/10 rounded-3xl">
              ...
              <p className="text-gray-400 text-center max-w-sm">Your buyer briefs will appear here once they have been set up by your agent.</p>
            </div>
          ) : (
            <>
          ...
          <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-5 mb-8 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
              {[
                ...
                <div key={idx} className={`${stat.bg} border border-white/10 px-4 py-3 rounded-xl min-w-[80px]`}>
                    ...
                    {filteredProperties.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            {(() => { const b = getStatusBadge(item); return (
                              <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${b.cls}`}>
                                {b.label}
                              </span>
                            ); })()}
                          </td>
