...
import {
  Bed, Bath, Car, Square, Calendar, Building2,
  ChevronLeft, Loader2, Play, FileText, Download,
  CheckCircle2, XCircle, AlertCircle, Check, X,
  ExternalLink, TrendingUp, DollarSign,
  Clock, MessageSquare, Eye, MapPin, Tag,
  Home, Waves, Ruler, BarChart2, PiggyBank, Percent,
  ArrowUpDown, Link2, BookOpen, LayoutList, Save, StickyNote
} from 'lucide-react';
...
const PropertyDetail = () => {
  ...
  const [documents, setDocuments] = useState({ propertyImages: [], images: [], videos: [], pdfs: [], others: [], externalVideos: [] });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (matchedItem) {
          setAssignment({ ...matchedItem.assignment, portalStatus: matchedItem.portalStatus });
          setClientNotes(matchedItem.clientNotes || matchedItem.assignment?.clientNotes || '');
        }
      } catch (error) {
    ...
    return (
      <Layout title="Property Not Found">
        <div className="text-center py-20">
          ...
          <button onClick={() => navigate('/client/dashboard')} className="text-teal hover:underline">
            Back to Dashboard
          </button>
        </div>
      </Layout>
  ...
  const weeklyRent = property.minRentPerMonth ? (property.minRentPerMonth / 4.33).toFixed(0) : null;
  const annualIncome = weeklyRent ? (weeklyRent * 52).toFixed(0) : null;
  const yieldMin = property.askingPriceMin ? ((annualIncome / property.askingPriceMin) * 100).toFixed(2) : null;
  const yieldMax = property.askingPriceMax ? ((annualIncome / property.askingPriceMax) * 100).toFixed(2) : null;
  ...
  const getDealProgress = () => {
    ...
    if (/offer.{0,15}(withdraw|reject)/.test(zs))
      return { completedCount: 3, terminal: { label: 'Offer Withdrawn by Seller' } };

    if (/\bdone\b/.test(zs) && !/bnp|finance|settle/.test(zs)) return { completedCount: 12, terminal: null };
    ...
    if (/contract.{0,5}sign/.test(zs))           return { completedCount: 5, terminal: null };
    if (/offer.{0,15}(accept|approv)/.test(zs))  return { completedCount: 4, terminal: null };
  ...
  const stageItems = (() => {
    ...
    for (let i = 0; i < DEAL_STAGES.length; i++) {
      ...
      const state = terminal
        ? (i < completedCount ? 'complete' : 'unreachable')
        : (i < completedCount ? 'complete'
          : i === completedCount && completedCount < 12 ? 'active'
          : 'pending');
    ...
    <Layout title={property.address}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {assignment && (
          <div className="flex flex-wrap items-center justify-between gap-4 mb-8 p-5 bg-navy border border-white/5 rounded-2xl">
            <div className="flex items-center gap-3">
              <span className="text-xs text-gray-500 uppercase tracking-widest font-bold">Assignment Status</span>
              <span className={`px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-widest border ${
                assignment.portalStatus === 'ACCEPTED'  ? 'bg-teal/10 text-teal border-teal/30' :
                assignment.portalStatus === 'REJECTED'  ? 'bg-red-500/10 text-red-400 border-red-500/30' :
                assignment.portalStatus === 'PURCHASED' ? 'bg-gold/10 text-gold border-gold/30' :
        ...
        <div className="space-y-12">
            ...
            {(documents.propertyImages.length > 0 || documents.videos.length > 0) && (() => {
              const galleryItems = [
              ...
              return (
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="md:col-span-3 aspect-[4/3] rounded-3xl overflow-hidden border border-teal/20 bg-navy">
                    {activeIsVideo
                      ? <video key={activeImage} controls autoPlay className="w-full h-full object-cover" src={activeImage} />
                      : <img src={activeImage} alt="Property" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    }
                  </div>
                  ...
                  <div className="flex md:flex-col gap-4 overflow-x-auto md:overflow-y-auto max-h-[500px] scrollbar-hide">
                    {galleryItems.map((item, idx) => {
                      ...
                      return (
                        <button
                          key={idx}
                          onClick={() => { setActiveImage(item.url); setActiveIsVideo(item.mediaType === 'video'); }}
                        ...
                        >
                          {item.mediaType === 'video'
                            ...
                            : <img src={item.url} alt={item.caption || item.fileName || ''} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                          }
                          {item.mediaType === 'video' && (
            ...
            <div className="bg-navy border border-teal/20 rounded-3xl overflow-hidden">
              ...
              <div className="px-8 py-5 bg-white/[0.03] border-b border-teal/20 flex items-center gap-3">
                ...
                <h3 className="text-lg font-bold text-white">Property Information</h3>
              </div>
              ...
              <div className="px-8 py-6 border-b border-white/5">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <p className="text-[10px] uppercase tracking-widest text-teal font-bold">Location & Identity</p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="sm:col-span-2 flex items-center gap-3 bg-white/[0.03] border border-white/5 rounded-xl px-4 py-3">
                    ...
                    <div>
                      <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-0.5">Address</p>
                      <p className="text-sm text-white font-semibold">{property.address || property.addressLine1 || '—'}</p>
                    </div>
                  </div>
                  ...
                  {[
                    ...
                    { label: 'LGA Region', value: property.lgaRegion, icon: MapPin   },
                    { label: 'Status',     value: property.status,    icon: Tag,
                      badge: true,
                      color: property.status?.toLowerCase().includes('active') ? 'text-teal bg-teal/10 border-teal/30'
                           : property.status?.toLowerCase().includes('sold')   ? 'text-gold bg-gold/10 border-gold/30'
              ...
              <div className="px-8 py-6 border-b border-white/5">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <p className="text-[10px] uppercase tracking-widest text-teal font-bold">Property Specs</p>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    { label: 'Car Spaces', value: property.carParking,                                             icon: Car        },
                    { label: 'Area',       value: property.areaSqm != null ? `${property.areaSqm} m²` : null,     icon: Ruler      },
                    { label: 'Year Built', value: property.yearBuilt,                                              icon: Calendar   },
                    { label: 'Pool',       value: property.pool === true ? 'Yes' : property.pool === false ? 'No' : null, icon: Waves },
                  ].map(({ label, value, icon: Icon }, i) => (
                    <div key={i} className="flex flex-col items-center justify-center gap-2 bg-white/[0.03] border border-white/5 rounded-xl px-3 py-4 text-center">
              ...
              <div className="px-8 py-6 border-b border-white/5">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <p className="text-[10px] uppercase tracking-widest text-teal font-bold">Financials & Sale Info</p>
                </div>

                {/* Price range highlight */}
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div className="bg-gold/5 border border-gold/20 rounded-xl px-4 py-4 text-center">
                    <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Min. Asking Price</p>
                    <p className="text-xl font-bold text-gold">
                      {property.askingPriceMin != null ? `$${Number(property.askingPriceMin).toLocaleString()}` : '—'}
                    </p>
                  </div>
                  <div className="bg-gold/5 border border-gold/20 rounded-xl px-4 py-4 text-center">
                    <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Max. Asking Price</p>
                    <p className="text-xl font-bold text-gold">
                      {property.askingPriceMax != null ? `$${Number(property.askingPriceMax).toLocaleString()}` : '—'}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {[
                    { label: 'Yield %',          value: property.yieldPercent != null ? `${property.yieldPercent}%` : null,                    icon: Percent,    color: 'text-teal' },
                    { label: 'Min Rent / Month',  value: property.minRentPerMonth != null ? `$${Number(property.minRentPerMonth).toLocaleString()}` : null, icon: PiggyBank,  color: 'text-teal' },
                    { label: 'Rental Appraisal',  value: property.rentalAppraisal,                                                              icon: BarChart2,  color: 'text-white' },
                    { label: 'Sale Type',         value: property.saleType,                                                                     icon: Tag,        color: 'text-white' },
                    { label: 'Rental Situation',  value: property.rentalSituation,                                                              icon: ArrowUpDown,color: 'text-white' },
              ...
              {/* ── Links ── */}
              {(property.linkToListing || property.stashLink || property.cmaLink) && (
                <div className="px-8 py-6">
                  <div className="flex items-center gap-2 mb-5">
                    <Link2 size={14} className="text-teal" />
                    <p className="text-[10px] uppercase tracking-widest text-teal font-bold">Links</p>
                  </div>
                  <div className="flex flex-wrap gap-3">
                    {property.linkToListing && (
                      <a href={property.linkToListing} target="_blank" rel="noopener noreferrer"
                    ...
                    {property.stashLink && (
                      <a href={property.stashLink} target="_blank" rel="noopener noreferrer"
                    ...
                    {property.cmaLink && (
                      <a href={property.cmaLink} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-2 px-5 py-3 bg-white/5 border border-white/10 hover:bg-white/10 text-gray-300 hover:text-white font-bold text-sm rounded-xl transition-all group">
                        <BarChart2 size={15} />
                        CMA Report
                      </a>
            ...
            <div className="bg-navy border border-teal/10 rounded-3xl p-8">
              ...
              <div className="space-y-10">
                {[stageItems.slice(0, 6), stageItems.slice(6)].filter(row => row.length > 0).map((row, rowIdx) => (
                  <div key={rowIdx} className="flex items-start">
                    {row.map((item, cellIdx) => {
                      ...
                      // Dot style
                      const dotClass = isTerminal          ? 'border-red-500 bg-red-500/20' :
                                       state === 'complete' ? 'border-teal bg-teal' :
                                       state === 'active'   ? 'border-teal bg-teal/10' :
                                       state === 'unreachable' ? 'border-gray-800 bg-white/[0.02]' :
                                       'border-gray-700 bg-white/[0.02]';

                      // Label style
                      const labelClass = isTerminal          ? 'text-red-400 font-bold' :
                                         state === 'complete' ? 'text-gray-400' :
                                         state === 'active'   ? 'text-white font-bold' :
                                         state === 'unreachable' ? 'text-gray-700' :
                                         'text-gray-500';

                      const isPurchasedMark = !isTerminal && item.idx === 7
                        && state !== 'unreachable' && state !== 'pending';
                      const cellOpacity = (state === 'unreachable' && !isTerminal) ? 'opacity-40' : '';
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center w-full">
                            ...
                            <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center shrink-0 ${dotClass}`}>
                              {isTerminal
                                ...
                                : state === 'complete'
                                ? <Check size={13} className="text-navy" strokeWidth={3} />
                                : state === 'active'
                                ? <div className="w-3 h-3 rounded-full bg-teal animate-pulse" />
                                : <span className="text-[9px] text-gray-600 font-bold">{item.idx + 1}</span>
                              }
                            </div>
                          ...
                          <div className="text-center mt-2.5 px-0.5 space-y-1">
                            <p className={`text-[10px] leading-tight ${labelClass}`}>{item.label}</p>
            ...
            <div className="bg-navy border border-teal/20 rounded-3xl p-8">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-xl font-bold text-white">Offer & Financial Analysis</h3>
                </div>
                <div className="px-4 py-1 bg-gold/10 border border-gold/30 rounded-full">
                  <p className="text-[10px] text-gold font-bold uppercase tracking-widest">Yield: {property.yieldPercent}%</p>
                </div>
              </div>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-widest mb-2">Min. Asking Price</p>
                  <p className="text-2xl font-bold text-white">${property.askingPriceMin?.toLocaleString() || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-widest mb-2">Max. Asking Price</p>
                  <p className="text-2xl font-bold text-white">${property.askingPriceMax?.toLocaleString() || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-widest mb-2">Weekly Rent</p>
                  <p className="text-2xl font-bold text-teal">${weeklyRent || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-widest mb-2">Annual Income</p>
                  <p className="text-2xl font-bold text-teal">${annualIncome || 'N/A'}</p>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-8 border-t border-white/5 pt-8">
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-widest mb-2">Yield (at Min Price)</p>
                  <p className="text-xl font-bold text-gold">{yieldMin ? `${yieldMin}%` : 'N/A'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-widest mb-2">Yield (at Max Price)</p>
                  <p className="text-xl font-bold text-gold">{yieldMax ? `${yieldMax}%` : 'N/A'}</p>
                </div>
              </div>
            ...
            {/* 5. Documents & Media */}
            {(() => {
              const hasAny = documents.videos.length > 0 || documents.images.length > 0
                || documents.pdfs.length > 0 || documents.others.length > 0
                || documents.externalVideos.length > 0
                || [assignment?.bnpReportLink, assignment?.financeLetterLink,
                    assignment?.contractDownloadLink, assignment?.docusignLink,
                    assignment?.cashflowDocLink].some(Boolean);
              ...
              return (
                <div className="space-y-8">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-bold text-white">Documents & Media</h3>
                  </div>
                        ...
                        {documents.videos.map((vid, idx) => (
                          <div key={idx} className="space-y-2">
                            <div className="aspect-video rounded-2xl overflow-hidden border border-teal/20 bg-navy">
                              <video controls className="w-full h-full" src={vid.url} />
                            </div>
                            <div className="flex items-center justify-between px-1">
                              <div className="min-w-0">
                                <p className="text-xs font-bold text-white leading-tight">{vid.caption || vid.fileName || '—'}</p>
                                <p className="text-[10px] text-teal/70 font-medium mt-0.5">
                                  {typeLabel(vid.documentType)}{fmtSize(vid.fileSizeBytes) ? ` · ${fmtSize(vid.fileSizeBytes)}` : ''}
                                </p>
                              </div>
                              {vid.url && (
                                <button onClick={() => handleDownload(vid.url, vid.fileName || vid.caption || 'video')}
                                  className="p-2 text-gray-500 hover:text-teal bg-white/5 rounded-lg hover:bg-white/10 transition-colors shrink-0 ml-2" title="Download">
                                  <Download size={15} />
                                </button>
                              )}
                            </div>
                      ...
                      <div className="space-y-4">
                        {documents.externalVideos.map((url, idx) => {
                          const embedUrl = url.includes('youtube.com/watch?v=')
                            ...
                            : null;
                          return embedUrl ? (
                            <div key={idx} className="aspect-video rounded-2xl overflow-hidden border border-teal/20 bg-navy">
                              <iframe width="100%" height="100%" src={embedUrl}
                                title={`Video ${idx + 1}`} frameBorder="0"
                            ...
                            <a key={idx} href={url} target="_blank" rel="noopener noreferrer"
                              ...
                              <div className="min-w-0 flex-1">
                                <p className="text-sm font-bold text-white">Property Video Link</p>
                        ...
                        {documents.images.map((img, idx) => (
                          <div key={idx} className="space-y-2">
                            <div
                              ...
                              onClick={() => setPreviewImg({ url: img.url, caption: img.caption || img.fileName })}
                            >
                              <img src={img.url} alt={img.caption || img.fileName || 'Image'}
                            ...
                            <div className="flex items-center justify-between px-1">
                              <div className="min-w-0">
                                <p className="text-xs font-bold text-white leading-tight">{img.caption || img.fileName || '—'}</p>
                                <p className="text-[10px] text-teal/70 font-medium mt-0.5">
                                  {typeLabel(img.documentType)}{fmtSize(img.fileSizeBytes) ? ` · ${fmtSize(img.fileSizeBytes)}` : ''}
                                </p>
                              </div>
                              {img.url && (
                                <div className="flex gap-2 shrink-0 ml-2">
                                  <button onClick={() => setPreviewImg({ url: img.url, caption: img.caption || img.fileName })}
                                    className="p-2 text-teal bg-teal/10 rounded-lg hover:bg-teal hover:text-navy transition-colors" title="View">
                                    <Eye size={15} />
                                  </button>
                                  <button onClick={() => handleDownload(img.url, img.fileName || img.caption || 'image')}
                                    className="p-2 text-gray-500 hover:text-teal bg-white/5 rounded-lg hover:bg-white/10 transition-colors" title="Download">
                                    <Download size={15} />
                                  </button>
                                </div>
                              )}
                      ...
                      <div className="space-y-2">
                        {[...documents.pdfs, ...documents.others].map((doc) => (
                          <div key={doc.id}
                            ...
                            <div className="min-w-0 flex-1">
                              <p className="text-sm font-bold text-white">{doc.caption || doc.fileName || '—'}</p>
                              <p className="text-[11px] text-teal/70 font-medium mt-0.5">
                                {typeLabel(doc.documentType)}{fmtSize(doc.fileSizeBytes) ? ` · ${fmtSize(doc.fileSizeBytes)}` : ''}
                              </p>
                            </div>
                            <div className="flex gap-2 shrink-0">
                              {doc.url && (
                                <button onClick={() => setPreviewDoc(doc.url)}
                                  className="p-2 text-teal bg-teal/10 rounded-lg hover:bg-teal hover:text-navy transition-colors" title="Preview">
                                  <Eye size={16} />
                                </button>
                              )}
                              {doc.url && (
                  ...
                  {[
                    ...
                    { label: 'Finance Letter',    link: assignment?.financeLetterLink },
                    { label: 'Contract',          link: assignment?.contractDownloadLink },
                    { label: 'DocuSign',          link: assignment?.docusignLink },
                    { label: 'Cashflow Analysis', link: assignment?.cashflowDocLink },
            ...
            <div className="bg-navy border border-teal/10 rounded-3xl p-8">
              ...
              {assignment?.agentNotes ? (
                <div className="p-6 bg-teal/5 border border-teal/20 rounded-2xl">
                  <p className="text-gray-300 leading-relaxed whitespace-pre-wrap">{assignment.agentNotes}</p>
                </div>
              ) : (
                <div className="p-10 border-2 border-dashed border-white/5 rounded-2xl flex flex-col items-center justify-center text-center">
                  <p className="text-gray-500 italic">Notes from your agent will appear here</p>
                </div>
              )}
            </div>
            ...
            {assignment && (
              <div className="bg-navy border border-teal/10 rounded-3xl p-8">
                ...
                <textarea
                  className="w-full bg-white/[0.03] border border-white/10 rounded-2xl p-4 text-sm text-gray-300 focus:border-teal outline-none transition-all resize-none h-36"
                ...
                <div className="flex justify-end mt-3">
                  ...
                  >
                    {savingNotes ? <Loader2 className="animate-spin" size={16} /> : <><Save size={15} /> Save Notes</>}
                  </button>
                </div>
              </div>
            )}
            ...
            <div className="bg-navy border border-teal/10 rounded-3xl p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-xl font-bold text-white">Property Notes</h3>
              </div>
              <div className="space-y-4">
                <p className="text-gray-400 leading-relaxed">
                  {property.notes || "No additional notes available for this property at this time."}
                </p>
              </div>
            </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => { setShowConfirmModal(null); setRemark(''); }}></div>
          <div className="relative bg-navy border border-teal/30 rounded-3xl p-8 max-w-md w-full shadow-2xl">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-6 mx-auto ${showConfirmModal === 'ACCEPT' ? 'bg-teal/20 text-teal' : 'bg-red-500/20 text-red-400'}`}>
              {showConfirmModal === 'ACCEPT' ? <CheckCircle2 size={32} /> : <AlertCircle size={32} />}
            </div>
            <h3 className="text-2xl font-bold text-white text-center mb-2">
              {showConfirmModal === 'ACCEPT' ? 'Express Interest?' : 'Not Interested?'}
            </h3>
            ...
            <div className="flex gap-4">
              <button
                onClick={() => { setShowConfirmModal(null); setRemark(''); }}
              ...
              >
                {actionLoading ? <Loader2 className="animate-spin" size={20} /> : 'Send to Agent'}
              </button>
            </div>
          </div>
        </div>
      ...
      {previewImg && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4" onClick={() => setPreviewImg(null)}>
          ...
          <div className="relative max-w-5xl w-full flex flex-col items-center gap-4" onClick={e => e.stopPropagation()}>
            <div className="w-full flex justify-between items-center px-2">
              <p className="text-white font-bold text-sm truncate max-w-[80%]">{previewImg.caption || 'Image Preview'}</p>
              <div className="flex gap-2">
                <button onClick={() => handleDownload(previewImg.url, previewImg.caption || 'image')}
                  className="p-2 text-teal bg-teal/10 rounded-xl hover:bg-teal hover:text-navy transition-colors" title="Download">
                  <Download size={18} />
                </button>
                <button onClick={() => setPreviewImg(null)}
                  className="p-2 text-gray-400 hover:text-white bg-white/10 rounded-xl transition-colors" title="Close">
                  <XCircle size={18} />
                </button>
              </div>
            </div>
            <div className="w-full max-h-[80vh] flex items-center justify-center rounded-2xl overflow-hidden border border-white/10">
              <img src={previewImg.url} alt={previewImg.caption || 'Preview'}
                className="max-w-full max-h-[80vh] object-contain" referrerPolicy="no-referrer" />
            </div>
          </div>
        </div>
      )}
      ...
      {previewDoc && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setPreviewDoc(null)}></div>
          <div className="relative bg-navy border border-teal/30 rounded-3xl p-4 w-full max-w-5xl h-[90vh] flex flex-col shadow-2xl">
            ...
            <div className="flex-1 rounded-2xl overflow-hidden bg-white/5 border border-white/10">
              <object data={previewDoc} type="application/pdf" className="w-full h-full">
                <div className="flex flex-col items-center justify-center h-full text-center p-8 space-y-4">
                  ...
                  <a href={previewDoc} target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-teal text-navy rounded-xl font-bold mt-4">
